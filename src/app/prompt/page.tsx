...
function PromptContent() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden p-4 md:p-6">
      ...
      <div className="relative z-10 w-full max-w-7xl flex flex-col" style={{ height: '90vh' }}>
        {/* Step badge */}
        <div className="mb-4 flex items-center">
          ...
          {hasPhoto && (
            <span className="ml-3 hidden md:flex items-center px-3 py-1.5 bg-amber-400/25 border border-amber-400/40 rounded-full text-amber-200 font-bold text-xs backdrop-blur-md">
        ...
        {/* Glass card */}
        <div className="flex-1 min-h-0 rounded-[2rem] border border-white/50 bg-white/15 p-5 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.4)] backdrop-blur-xl md:p-6 flex flex-col">
          {/* Header */}
          <div className="mb-4 shrink-0">
            <h1 className="text-xl md:text-2xl font-extrabold text-white flex items-center drop-shadow">
          ...
          <div className="flex flex-col lg:flex-row gap-5 flex-1 min-h-0">
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-4 custom-scrollbar">
              ...
              <div className="bg-black/20 p-4 rounded-2xl border border-white/15">
                <div className="flex items-center justify-between mb-3">
                  <label className="text-white font-bold text-sm flex items-center">
                    <ImageIcon className="w-4 h-4 text-amber-300 mr-2" />
                    대표 화풍 예시 사진 (터치해서 바로 선택!)
                  </label>
                  <span className="text-xs text-white/50">사진을 누르면 화풍이 바뀝니다</span>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected
                            ? "border-amber-400 ring-2 ring-amber-400/50 scale-[1.03] shadow-[0_0_20px_rgba(251,191,36,0.4)]"
                            : "border-white/15 bg-black/30 hover:border-white/30 hover:scale-[1.02]"
                        )}
                      >
                        <div className="aspect-square w-full relative overflow-hidden bg-slate-800">
                          ...
                          {isSelected && (
                            <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-amber-400 text-white flex items-center justify-center shadow-lg">
                              <Check className="w-3 h-3 stroke-[3]" />
                            </div>
                          )}
                          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-2 pt-4">
                            <span className="text-white font-bold text-xs block truncate drop-shadow">
                              {opt.label}
                            </span>
                          </div>
                        </div>
              ...
              {(Object.keys(CATEGORY_DATA) as Array<keyof Selections>).map((key) => {
                ...
                return (
                  <div key={key} className="flex flex-col bg-black/20 p-3.5 rounded-xl border border-white/15 hover:border-white/25 transition-colors">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-white/90 text-sm font-bold flex items-center">
                        <span className="w-2 h-2 rounded-full bg-amber-400 mr-2 shrink-0" />
                        {cat.label}
                      </label>
                      {selectedOpt?.desc && (
                        <span className="text-xs text-amber-300/80 truncate max-w-[180px]">
                          {selectedOpt.desc}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full appearance-none bg-black/40 border border-white/20 text-white text-sm font-medium rounded-lg px-3 py-2.5 outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-all cursor-pointer hover:bg-black/60"
                      >
                        {cat.options.map((opt) => (
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              {activeMediumItem && activeMediumItem.image && (
                <div className="bg-gradient-to-r from-amber-900/30 to-orange-900/30 border border-amber-500/30 rounded-2xl p-4 flex items-center gap-4 shrink-0">
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="inline-block px-2 py-0.5 rounded-full bg-amber-500/30 text-amber-200 text-xs font-bold mb-1">
                      현재 선택된 그림체
                    </div>
                    <h3 className="text-base font-bold text-white truncate">{activeMediumItem.label}</h3>
                    <p className="text-xs text-white/60 truncate">{activeMediumItem.desc}</p>
                  </div>
                </div>
              )}

              {/* Prompt Display Box */}
              <div className="bg-black/30 rounded-2xl p-4 flex-1 flex flex-col border border-white/15">
                <div className="text-white/80 font-bold text-sm mb-3 flex justify-between items-center">
                  ...
                  <button
                    onClick={handleReset}
                    className="text-white/50 hover:text-white flex items-center text-xs transition-colors font-normal px-2.5 py-1 bg-white/5 rounded-lg hover:bg-white/10 cursor-pointer"
                  >
                ...
                <textarea
                  ...
                  className="w-full flex-1 bg-transparent text-white text-sm md:text-base outline-none resize-none placeholder-white/30 leading-relaxed font-normal"
              ...
              <div className="flex gap-3 shrink-0">
                <button
                  onClick={handlePrev}
                  className="flex items-center justify-center px-5 py-4 rounded-2xl border border-white/40 bg-white/10 text-white font-bold text-sm hover:bg-white/20 transition-colors cursor-pointer backdrop-blur"
                >
                ...
                <button
                  ...
                  disabled={!prompt}
                  className={cn(
                    "flex-1 flex items-center justify-center py-4 text-lg font-bold rounded-2xl transition-all duration-300",
                    prompt
                      ? "bg-gradient-to-r from-amber-400 via-orange-400 to-amber-500 text-white hover:scale-[1.02] shadow-[0_0_30px_rgba(251,191,36,0.4)] cursor-pointer"
